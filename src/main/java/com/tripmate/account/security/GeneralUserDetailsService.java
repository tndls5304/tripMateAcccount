...
public class GeneralUserDetailsService implements UserDetailsService {
    ...
    /**
     *userEntity의 getRoles() 메서드를 통해 사용자의 권한(예: ROLE_ADMIN, ROLE_USER 등)을 가져오고, 이를 시큐리티가 권한을 비교할때 사용하는 SimpleGrantedAuthority 객체로 변환해 반환합니다.
     * map(Role::getAuthority)는 각 Role 객체에서 권한 문자열을 가져오고, 그 문자열을 SimpleGrantedAuthority 객체로 변환해 최종적으로 권한 목록을 생성합니다
     * @param userEntity
     * @return
     */
    private Collection<? extends GrantedAuthority> getAuthorities(UserEntity userEntity) {
        return userEntity.getRoles().stream()
                .map(Role::getAuthority)//getAuthority() 메서드를 호출하여 각 역할에서 권한 문자열을 가져옵니다."ROLE_ADMIN" 또는 "ROLE_USER"와 같은 문자열을 반환한다고 가정할 수 있습니다.
