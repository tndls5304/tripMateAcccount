...
import static com.tripmate.account.common.errorcode.CommonErrorCode.JWT_ERROR_ROLE_BLANK;
...
public class JwtAuthService {
    ...
    /**
     * 로그인 시 JWT 를 생성하는 메서드
     * 클라이언트에게는 토큰유무와 상관 없이 access 토큰과 refresh 토큰을 모두 새로 발급하며,
     * DB에는 access토큰을 저장하지 않고 refresh token만 저장합니다.
     *
     * @param clientId    로그인 성공 후 authentication의 사용자 id//TODO
     * @param clientRoles 로그인 성공 후 authentication의 사용자 권한리스트
     * @return 생성된 JWT 토큰
     */
    ...
    /**
     * 사용자 ID와 역할 정보를 바탕으로 DB에 저장할 키를 생성하는 메서드
     * 사용자 id마다 권한을 여러개 가질 수 있지만 자신의 계정을 나타내는 필수 권한은 다 가지고 있어야 함
     * 게스트는 "RG00" ,호스트는 "RH00" , 관리자는 "RA00"
     *
     * @param userId 게스트 or 호스트 or 관리자 id
     * @param roles  게스트 or 호스트 or 관리자 권한목록
     * @return 사용자의 id와 필수 권한정보로 만들어진 조합키
     */
    private String createKeyOfDb(String userId, List<String> roles) {
        if (roles.contains("RG00")) {
            ...
            throw new MissingRoleException(JWT_ERROR_ROLE_BLANK);
    }
    ...
    public JwtToken reissueJwtToken(String clientAccessToken, String clientRefreshToken) {
        ...
        List<String> clientRoles = reqRefreshClaims.get(ROLE_CLAIM, List.class);
    ...
    private String createServerAccessToken(Claims serverRefreshClaims, Claims clientAccessClaims) {
        ...
        List<String> serverSavedRoles = serverRefreshClaims.get(ROLE_CLAIM, List.class);
        ...
        return Jwts.builder()
                ...
                .claim(ROLE_CLAIM, serverSavedRoles)
